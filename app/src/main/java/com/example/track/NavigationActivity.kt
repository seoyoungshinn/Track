...
class NavigationActivity : Activity(), LocationListener {
    ...
    //mqtt관련
    var server_uri = "tcp://172.20.10.2:1883"
    ...
    val gpsLocationListener: LocationListener = object : LocationListener {
        override fun onLocationChanged(location: Location) {
            ...
            if(midpointList.size!=0) { //NavigationActivity가 겹치는 문제를 해결
                ...
                Log.d(LOG,"다음위치 : ["+"${midpointList[midPointNum][0]}"+", "+"${midpointList[midPointNum][1]}"+"]")
                Log.d(LOG,"midpointNum : ["+"${midPointNum}"+"]")
                ...
                //분기점일때
                else if ((turnTypeList[midPointNum] >= 212 || (turnTypeList[midPointNum] in 12..19)) && sppoint != 0) {
                    vibe(1000,150)
                    sppoint = 0
                    turnPointCount++
                ...
                //경로이탈인지 아닌지 판단
                if (setting!=0 && (DetailRoute.getDistance( lat, lon, midpointList[midPointNum][0], midpointList[midPointNum][1]) > 20.0)) {  //p1에서 멀어졌는데
                    if (DetailRoute.getDistance( lat, lon, midpointList[midPointNum + 1][0], midpointList[midPointNum + 1][1]) > 20.0) {  //p2에서도 멀어졌다.
                        outNum++
                        ...
                        if (outNum > 15) {
                            ...
                            turnTypeList.clear()
                            val returnIntent = Intent()
                            returnIntent.putExtra("destination",destination)  //목적지를 main에 넘기고 그 값을 다시 DoRetrofit으로 넘긴다.
